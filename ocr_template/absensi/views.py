...
from .forms import AbsensiForm, DateRangeForm
...
def admin_panel(request):
    ...
    try:
        absensi_list = AbsensiTamu.objects.all()
    except (OperationalError, ProgrammingError) as e:
        absensi_list = []
...
def download_pdf_rekap_hari_ini(request):
    ...
    if absensi_hari_ini.exists():
        ...
        header_data = [
            ...
            Paragraph('Keperluan', header_cell_style),
            Paragraph('Waktu', header_cell_style)
        ]
        data = [header_data]
        ...
        for idx, absensi in enumerate(absensi_hari_ini, start=1):
            ...
            waktu = absensi.tanggal_waktu.strftime('%H:%M')
        ...
        table = Table(data, colWidths=[0.5*inch, 1.6*inch, 1.6*inch, 3.0*inch, 0.6*inch])
...
def download_excel_rekap_hari_ini(request):
    ...
    headers = ['No', 'Nama Lengkap', 'Instansi/Asal', 'Keperluan', 'Waktu']
    ...
    if absensi_hari_ini.exists():
        for idx, absensi in enumerate(absensi_hari_ini, start=1):
            ...
            # Instansi/Asal
            ws.cell(row=row_num, column=3, value=absensi.instansi if absensi.instansi else '-')
            
            # Keperluan
            ws.cell(row=row_num, column=4, value=absensi.keperluan)
            
            # Waktu
            ws.cell(row=row_num, column=5, value=absensi.tanggal_waktu.strftime('%H:%M'))
    ...
    column_widths = [8, 25, 30, 50, 12]  # Width in characters
